...
int
poly1305_auth_verify(const unsigned char mac1[16], const unsigned char *m, size_t bytes, const unsigned char key[32]) {
  unsigned char mac2[16];
  poly1305_auth(mac2, m, bytes, key);
  return (1 == poly1305_verify(mac1, mac2)) ? 0 : 1;
}
